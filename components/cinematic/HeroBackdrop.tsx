...
/**
 * The animated glass-and-network backdrop behind a hero, drawn live on a canvas.
...
export function HeroBackdrop({
  ...
  useEffect(() => {
    // A page that has a lot to do first can hold the animation back a moment; the still poster covers the gap.
    ...
    function boot(): (() => void) | undefined {
      ...
      measure();
      const scene = createNetworkScene(ctx, w, h);
      ...
      const sync = () =>
        onScreen && document.visibilityState === "visible" ? start() : stop();
  ...
  return (
    <div
      aria-hidden
      className="absolute inset-0 -z-20 overflow-hidden bg-obsidian"
    >
