...
**Still to do:** the production-agent pipeline (Phase 8) needs a way to roll a build back to an earlier version and a written QA checklist, and every customer-facing route still needs its own isolation test. Everything else in the plan is built.
