...
| 22 | Checkout with personalized thank-you steps | **Done** | The six-step "You just took the first step" stepper (website, project, and card variants) is paired with a walkthrough (`components/checkout/ThankYouCard.tsx`) that states, for every step, who it is waiting on (you or us) and real timing: the same 72-hour target the live status page uses for a website, and the bought product's actual delivery window and revision count for everything else. Each kind ends with a plain "how you'll know it's done" line and a real phone number and email. **Deposits and invoices are now built** (Phase 4): orders over $1,500 that ship nothing can start with a 50% deposit; production starts on the deposit; the balance is invoiced automatically when the build is ready and delivery (or a website launch) waits for it; the owner can bill extra work from /admin/invoices; customers get private pay links, receipts, and a portal Invoices page. The deposit percent and minimum live in `DEPOSIT` in the price catalog. |
| 23 | Contextual upsells | **Done (v1)** | See item 11. Never offers what is owned; consultations get none; clicks and views are counted. |
| 24 to 25 | Production pipeline with human approval and rollback | **Done (v1)** | Agents, QA, and approval exist. The owner can now go back to any earlier website version before launch (it becomes a new version the customer approves again; nothing is deleted; reason recorded), and "Mark as live" is gated by a launch checklist: approved, no open change request, automated checks passed, intake done, paid in full, plus four checks only the owner can tick per version. "Launch anyway" is possible but recorded. Written in `docs/PRODUCTION_QA_CHECKLIST.md`. |
...
| 32 | Security | **Done (v1)** | Roles, signature checks, rate limits, and private tokens exist, and a test now makes every API route declare who may call it (owner, signed-in customer, private link, server secret, or open with the reason written) and checks the code holds that guard. Isolation tests cover the portal, every private link, and the message thread. That work found and closed one real gap: the order confirmation page and card-details form were open to anyone who had an order number; they now need a private key that only the buyer's link carries. |
| 33 | Founder operating dashboard and the morning brief | **Done (v1)** | /admin/founder (new "Founder" tab): Vision, Sales, Relationships, Systems (automation health and which services are switched on), Customer experience, Financials, and the Bottleneck. The Founder Operating Brief answers the eight questions from real data (yesterday, what produced revenue, what failed, what needs attention, which leads to follow up, which customers to upsell, which partnerships need attention, and the single highest-leverage task, chosen in a fixed order: a stuck customer, a waiting customer, owed money, an overdue lead, then the bottleneck). Optional morning email at /api/cron/founder-brief, off unless CRON_SECRET is set. No numerology: priorities come only from counts and dates. |
| 34 | Anti-distraction: the Business Idea Parking Lot | **Done (v1)** | /admin/founder/ideas: save an idea, answer the seven questions, get a verdict. Saving never starts anything. Starting is refused until all questions are answered, at most two ideas can be in progress, and an idea that distracts from the current bottleneck needs an explicit override. |
| 35 | Bottleneck-first strategy | **Done (v1)** | The path is checked from the top (traffic, conversion, fulfillment, keeping customers, selling more) and the first broken stage is the constraint. Every stage shows the number it measured and the line it was measured against; the lines are in one place (BOTTLENECK_LINES) and are rules of thumb. |
| 36 | Product profitability | **Done (v1)** | /admin/founder/profit: revenue less payment fees, partner and referral commissions, refunds, and the costs you enter (fulfillment, AI and API, your time at your hourly value, software) gives contribution margin. Real agent run costs are used when recorded. Products are ranked in seven ways (revenue, contribution, margin, margin per labor hour, conversion, retention, upsell) and overall by average rank. A product with no costs entered is left out of the margin rankings, never treated as free. |
...
| 2026-09-24 | Agent Universe | A universal multi-agent layer (MASTER plus ten agents) that reads the real records and pages, ranks what to investigate by the founder's own constraint order, and hands the owner a plan. MASTER wakes only the agents a request needs. Owner dashboard at Admin, Agents. Never changes prices, sends email, or spends money; everything else is recorded and confirmed. Optional daily run, off until a scheduler, CRON_SECRET, and the AUTONOMOUS level are set. Patient Creations is the first domain. 102 new tests, 993 in all. Specification in `agent-universe/`. |
| 2026-09-22 | Post-purchase walkthrough | The confirmation page's thank-you card now spells out every step's timing and who it's waiting on, from the order's real turnaround and revision count, plus a "how you'll know it's done" line. 8 new tests, 891 in all. |

**Still to do:** nothing in the plan is unbuilt. What remains is owner-side: live payment paths only tested with a fake Stripe, the email domain, the remaining voice lines, a lawyer's read of the deposit and partner terms, and changing the default admin password.

| 2026-09-21 | 8. Rollback, launch checklist, isolation | Go back to an earlier website version, a launch checklist gating "Mark as live", the written QA checklist, a route-by-route access policy test, isolation tests for the portal and private links, and a private key on order confirmations. 60 new tests, 883 in all. |
| 2026-09-21 | Founder dashboard, ideas, bottleneck, profitability | The Founder Operating Brief, the bottleneck rule, the Idea Parking Lot with its limits, and product profitability with owner-entered costs. 62 new tests, 813 in all. |
...
**Test and check counts:** 813 unit tests; homepage, finder, audit, and confirmation page also checked in a real browser at phone and desktop widths.
