...
import { AD_SPECIAL_BLURB, AD_SPECIAL_INCLUDES, BUNDLE_DESCRIPTION, BUNDLE_ITEMS, CINEMATIC_SPECIAL_DESCRIPTION, LEAD_ENGINE_DESCRIPTION, UGC_SPECIAL_DESCRIPTION } from "@/lib/site/adSpecials";

const all = () => [...Object.values(AD_SPECIAL_INCLUDES), ...Object.values(AD_SPECIAL_BLURB), CINEMATIC_SPECIAL_DESCRIPTION, UGC_SPECIAL_DESCRIPTION, BUNDLE_DESCRIPTION, ...BUNDLE_ITEMS, LEAD_ENGINE_DESCRIPTION].join("\n");
...
describe("ad specials", () => {
  ...
  it("are the same words on the card, in the product rows, and in the bundle", () => {
    ...
    for (const name of ["CINEMATIC_SPECIAL_DESCRIPTION", "UGC_SPECIAL_DESCRIPTION", "BUNDLE_DESCRIPTION", "LEAD_ENGINE_DESCRIPTION"]) expect(seed, name).toContain(`description: ${name}`);
  });
